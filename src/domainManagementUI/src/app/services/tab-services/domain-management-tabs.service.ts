...
export class DomainManagementTabService{
    ...
    public domain_data_behavior_subject: BehaviorSubject<DomainModel> = new BehaviorSubject<DomainModel>(new DomainModel())
    ...
    constructor(
        ...
        this._rebuildForms()
        this.domain_data_behavior_subject.subscribe(
            (data) => {
    ...
    _rebuildForms(){
        this.gen_attribute_tab_form = new FormGroup(
            {
                application_uuid: new FormControl('', {validators: Validators.required}),
     ...
     */
    _onChanges(): void {
        this.gen_attribute_tab_form.controls.application_uuid.valueChanges.subscribe((val) => {
            console.log(val);
